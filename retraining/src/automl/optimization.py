    ...
    if config["regression"]["encoder"]:
        ...
        neuron_count_per_hidden_layer = [
            ...
            int(
                ...
                (2 ** int(config["regression"]["num_neurons"]))
                / (
                    ...
                    ** (
                        ...
                        index
                        if index < (int(config["regression"]["num_hidden_layers"]) / 2)
                        # the inverted position (index) in the list (network) IF IT IS IN THE SECOND HALF
                        else int(config["regression"]["num_hidden_layers"]) - 1 - index
                    )
                )
            )
            for index in range(int(config["regression"]["num_hidden_layers"]))
        ]
    # If not encoding
    else:
        ...
        neuron_count_per_hidden_layer = [
            2 ** int(config["regression"]["num_neurons"])
        ] * int(config["regression"]["num_hidden_layers"])
    ...
    return (
        ...
        and (
            (
                ...
                and int(config["regression"]["num_hidden_layers"]) < 4
            )
            or (
                config["regression"]["encoder"]
                and int(config["regression"]["num_hidden_layers"]) % 2 == 1
                and int(config["regression"]["num_neurons"]) > 6
            )
        )
...
def keras_objective(X_train, y_train, X_val, y_val, X_test, y_test, metric, seed, config, re_training_offset, start_timestamp):
    ...
    dnn = build_dnn(
        ...
        float(config["regression"]["dropout"]),
        X_scaler,
        y_scaler,
    )
    ...
    early_stop = keras.callbacks.EarlyStopping(
        monitor="val_loss",
        patience=(2 ** int(config["regression"]["num_epochs"])) // 5,
    ...
    if config["regression"]["optimizer"] == "SGD":  # non-adaptive optimizer
        ...
        reduce_lr = keras.callbacks.ReduceLROnPlateau(
            monitor="val_loss",
            factor=0.8,
            patience=(2 ** int(config["regression"]["num_epochs"])) // 10,
            mode="min",
            min_lr=0.000001,
        )
    ...
    dnn.fit(
        ...
        epochs=2 ** int(config["regression"]["num_epochs"]),
        batch_size=2 ** int(config["regression"]["batch_size"]),
        shuffle=False,
        callbacks=callbacks,
    )

    print("LR after first training: : ", optimizer.lr.numpy())

    ### RE-TRAINING PROCESS ###
    re_training_window = 25 # number of days considered for each re-training
    re_training_offset_counter = 0

    if re_training_offset > 0:
        optimizer = globals()[config["regression"]["optimizer"]](0.0001) # restart from a smaller lr
        dnn.compile(
            loss=metric_function,
            optimizer=optimizer,
            metrics=[metric_function],
        )
    while re_training_offset_counter < re_training_offset:
        # Instantiate optimizer and callbacks for re-training
        ...
        early_stop = keras.callbacks.EarlyStopping(
            monitor="val_loss",
            patience=(2 ** int(config["regression"]["num_epochs"] - 1)) // 5,
            restore_best_weights=True,
    	)
        callbacks.append(early_stop)
        if config["regression"]["optimizer"] == "SGD":  # non-adaptive optimizer
            reduce_lr = keras.callbacks.ReduceLROnPlateau(
                monitor="val_loss",
                factor=0.8,
                patience=(2 ** int(config["regression"]["num_epochs"] - 1)) // 10,
                mode="min",
                min_lr=0.000001,
            )
            callbacks.append(reduce_lr)
        ...
        """
        #print(X_test.loc[start_timestamp + ((re_training_offset_counter - re_training_window) * 24 * 60 * 60):start_timestamp + (re_training_offset_counter * 24 * 60 * 60), ].to_numpy())
        print("RE-TRAINING")
        dnn.fit(X_test.loc[start_timestamp + (re_training_offset_counter * 24 * 60 * 60):start_timestamp + ((re_training_offset_counter + re_training_window) * 24 * 60 * 60), ].to_numpy(),
                y_test.loc[start_timestamp + (re_training_offset_counter * 24 * 60 * 60):start_timestamp + ((re_training_offset_counter + re_training_window) * 24 * 60 * 60), ].to_numpy(),
                #validation_data=(X_val.to_numpy(), y_val.to_numpy()), ### se voglio applicare early stopping anche nel retrain, ho bisogno di validation data
                epochs=256, # 2 ** int(config["regression"]["num_epochs"] - 1),
                batch_size=2 ** int(config["regression"]["batch_size"]), #
                shuffle=False,
                #callbacks=callbacks, #
        )
        re_training_offset_counter += re_training_window

    # Prediciton
    if re_training_offset > 0:
        y_train_pred = dnn.predict(X_test.loc[start_timestamp:start_timestamp + (re_training_offset * 24 * 60 * 60), ].to_numpy())
        #print("SIZE y_train_pred: ", y_train_pred.shape)
        #print("SIZE y_train_concat: ", pd.concat([X_train] + [X_test.loc[start_timestamp:start_timestamp + (re_training_offset * 24 * 60 * 60), ]], axis=0).shape)
    else:
        y_train_pred = dnn.predict(X_train.to_numpy())
    y_val_pred = dnn.predict(X_val.to_numpy()) # forse inutile
...
def objective(
    ...
    try:
        ...
        if re_training_offset > 0:
            #print("SIZE y_train_pred: ", y_train_pred.shape)
            #print("SIZE y_train_concat: ", pd.concat([y_train] + [y_test.loc[start_timestamp:start_timestamp + (re_training_offset * 24 * 60 * 60), ]], axis=0).shape)
            y_train_pred_df = pd.DataFrame(
                y_train_pred, columns=y_test.loc[start_timestamp:start_timestamp + (re_training_offset * 24 * 60 * 60), ].columns, index=y_test.loc[start_timestamp:start_timestamp + (re_training_offset * 24 * 60 * 60), ].index
            )
        else:
            y_train_pred_df = pd.DataFrame(
        ...
        train_raw_rmse = metric_function(
            y_test.loc[start_timestamp:start_timestamp + (re_training_offset * 24 * 60 * 60), ] if re_training_offset > 0 else y_train, y_train_pred_df, multioutput="raw_values", squared=False
        )
        ...
        result["train_score"] = np.around(
            metric_function(y_test.loc[start_timestamp:start_timestamp + (re_training_offset * 24 * 60 * 60), ] if re_training_offset > 0 else y_train, y_train_pred_df, squared=False), 2
        )
